...
const UILayout: FC<UILayoutProps> = ({ children, sideMenu, header, handlerRef }) => {
  ...
  const content = (
    <div className="flex flex-col h-dvh">
      {header && (
        <header className="bg-base-100 text-base-content flex-none z-20 shadow h-16 w-full justify-center border-b border-gray-200 duration-100">
  ...
  return (
    <div className="bg-base-100 drawer lg:drawer-open animate-fadein">
      ...
      <div className="drawer-side overflow-hidden z-40 border-r border-base-300 shadow-lg">
        ...
        <aside className={cn("bg-base-100 min-h-screen flex flex-col", sideMenu.className)} style={sideMenu.style}>
          {sideMenu.header && (
            <nav className="flex-none bg-base-100 w-full border-b border-base-300 gap-y-2 h-16 flex">
